...
Shader "Unity Shaders Book/Common/JayOriBumped Diffuse" {
	Properties {
	...
	SubShader {
		...
		Pass { 
			...
			fixed4 frag(v2f i) : SV_Target {
				...
				float4 clipPos_devided = i.pos/i.pos.w;
				float4 customClipPos = mul(unity_MatrixVP,float4(worldPos,1));
				// customClipPos /= customClipPos.w;
				// clipPos_devide_w.xy = clipPos_devide_w.xy; 
			 
				// fixed4 pos01 = fixed4(0.5*i.pos.x+0.5,0.5*i.pos.y+0.5,0.5*i.pos.z+0.5,1);
