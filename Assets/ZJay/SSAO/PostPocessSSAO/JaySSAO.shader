...
///坑点记录：
// - 采样点的z和采样点对应的深度depth比较，而不是shadering point的。
// - camera的near要够大，否则异常。 根本原因待查。
...
//			Tags { "LightMode"="ForwardBase" }
			...
			for (int ii = 0; ii < _SampleCount; ++ii)
			{
			   ...
			   if(Linear01Depth(uvz_sample.z) < Linear01Depth(sample_depth))//
			   {
			      count_vis+=1;
			   }
			   if(ii==11)
				...
				float3 offset = float3(0,_FloatTest,0);
			   float4 pos_world_sample = float4(pos_world.xyz+offset,1);
			   float4 pos_clip_sample = mul(_CurrentViewProjectionMatrix, pos_world_sample);
			   //[-1,1]
			   float4 pos_clip_devided = pos_clip_sample/pos_clip_sample.w;
			...
			// v_average =Linear01Depth(sample_depth) ;
			// v_average = Linear01Depth(testDepth);;
			// v_average = Linear01Depth(depth) ;
			// v_average = sample_depth ;
